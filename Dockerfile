...
RUN apt-get install -y libsasl2-dev python-dev libldap2-dev libssl-dev  libpq-dev
...
ADD . /code/
WORKDIR /code/src/
RUN cd django-crispy-forms && pip install -e .
RUN cd django-helpdesk && pip install -e .
